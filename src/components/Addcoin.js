	...
	handleSubmit = event => {
		...
		if (this.validator.allValid()) {
			const data = {
				...
				presale_date: this.state.presale_date,
			...
			axios.post( this.state.apiLocalURL+'coins', { data, file : this.state.logo }).then( res =>{
				console.log(res);
						...
						<div id="utf_add_listing_part">     
							...
							<div className="add_utf_listing_section margin-top-45"> 
								...
								<div className="row with-forms">
									...
									<div className="col-md-6">
										<h5>Presale <input type="checkbox" name="presale" id="presale" className="search-field" style={{width:'3%',height:'15px'}} onChange={(event) => this.handleUserInput(event)} /></h5>
										<input type="date" className="search-field" name="presale_date" id="presale_date" placeholder="Presale Date" value={this.state.presale_date} onChange={(event) => this.handleUserInput(event)} style={this.state.showPresale ? {} : { display: 'none' }} />
										{this.validator.message('presale_date', this.state.presale_date, this.state.showPresale ? 'required|dateFormat' : '' )}
									</div> 
								</div>
							</div>
							<div className="add_utf_listing_section margin-top-45"> 
